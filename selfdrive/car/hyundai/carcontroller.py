    ...
    if self.CP.carFingerprint in CANFD_CAR:
      ...
      if not self.CP.openpilotLongitudinalControl:
        ...
        if not CC.cruiseControl.resume and CS.out.cruiseState.enabled and not CS.out.gasPressed and not self.CP.pcmCruiseSpeed:
          ...
          if cruise_button is not None:
            can_sends.append(hyundaican.create_clu11(self.packer, self.resume_count, CS.clu11, cruise_button, self.CP.carFingerprint))
            self.resume_count += 1
          else:
            self.resume_count = 0
      ...
      if self.frame % 2 == 0 and self.CP.openpilotLongitudinalControl:
        ...
        can_sends.extend(hyundaican.create_acc_commands(self.packer, CC.enabled and CS.out.cruiseState.enabled and CS.mainEnabled, accel, jerk, int(self.frame / 2),
                                                          hud_control.leadVisible, set_speed_in_units, stopping, CS.out.gasPressed, CS.mainEnabled, enhanced_scc, tau_gap_set, CS))
        self.accel = accel
